...
- Test Test Diye Nice Nice: Mocklama - 1: Fonksiyon Mocklama (Şu anda
buradasınız!)
- [Test Test Diye Nice Nice: Mocklama - 2: Modül Mocklama](/blog/frontend/test-test-diye-nice-nice-2-modul-mocklama)
- [Test Test Diye Nice Nice: Mocklama - 3: Timer Mocklama](/blog/frontend/test-test-diye-nice-nice-3-timer-mocklama)
...
<CustomImage
  src="/images/post/frontend/test-test-diye-nice-nice-1-fonksiyon-mocklama/002.png"
  width={996}
  height={ 383}
  fluid
  borderless
...
de zamanı gelmişken `mockRestore` ile nasıl çalıştığını görelim:
...
Mock fonksiyonlarının neyle ve kaç kez çağrıldığını test etmek için `.mock` 
özelliğini kullanabiliriz. Fakat Jest bu değerleri manual kontrol etmek yerine 
...
test("playground", async () => {
  const calledArguments = [
    [1, 2, 3],
    { name: "Enes", surname: "Başpınar", job: "Software Developer", age: 23 },
  ];
...
<CustomImage
  src="/images/post/frontend/test-test-diye-nice-nice-1-fonksiyon-mocklama/003.png"
  width={1140}
  height={1139}
  fluid
  borderless 
