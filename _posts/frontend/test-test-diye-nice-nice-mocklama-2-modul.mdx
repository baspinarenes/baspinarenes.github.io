...
Aha, yine hata. `get` metodunu mocklayamadık. İlgili dosyanın fonksiyonları 
yalnızca import edildiği dosyalarda mocklanır, kendi dosyasının içerisinde 
kullanıyorsak mocklanmaz. Peki biz `utils.ts`'de `./utils` dosyasının import 
ediyor muyuz? Hayır! Teyit etmek için `mockedUtils` değişkenini yazdıralım.
