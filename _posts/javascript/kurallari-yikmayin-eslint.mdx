...
date: 2022-04-15
summary: "Bize sürekli hata fırlatan ancak her şeye rağmen asla vazgeçemediğimiz ESLint'in ne olduğuna birlikte bakalım."
---

<CustomImage src="/images/post/kurallari-yikmayin-eslint/001.webp" width={1080} height={652} />
...
> _Dikkat Tanım Var!_ Az sonra bir tanımla karşılaşacaksınız. Çok üzerinde
> durmadan arkalara doğru ilerleyelim. Bizim işimiz tanım ezberlemek değil, işin
> özüne dair fikir sahibi olmak.
...
çalmaz, okunması ve debug edilmesi kolaylaşır. Mesela Allahlık bir kodun
ESLint'ten elinden geçtikten sonraki halini görelim:
...
Sağladığı önemli şeylerden birisi de kodu çalıştırmadan önce belirli bugları
tespit edebilmesidir. Örneğin aşağıdaki gibi `for` içinde [değişkeni deklare etmeyi
unutursanız](focus://3[6:12]) ya da 100'den geriye saymayı amaçladığınız bir kodda
...
İstenmeyen durumlara karşı tavrı nettir. Bir gün taskınızı bitirmişsinizdir
arkanıza yaslanıp commit atarsınız ancak o da ne ESLint karşınızda:

<CustomImage src="/images/post/kurallari-yikmayin-eslint/002.webp" width={715} height={429} />

Bu arada kitabi tanımını her yerde bulabilirsiniz ancak kimin umrunda. Tanımlarda boğulmak istemiyoruz. Örneklendirdiğimizde kafamızda zaten netleşecek.
...
adına [ilgili satırı](focus://10) silerek diğer ayarları kapatacağız.

> Blog yazısı içerisinde altı noktalı metinlere denk gelirseniz üzerinde durun.
> Belirttiğim kısımların kod bloğundaki yerini gösteriyor olacağım.
...
`package.json` içerisinde `eslintConfig` kullanarak da konfigürasyonumuzu yapabiliriz.
...
```
Peki ne tür değerler atayabiliriz? Öncelikle davranışlarından bahsedelim. Üç
...
Farklı kullanımlara sahip bazı örnekleri inceleyelim.
...
Neyse basit senaryomuza geri dönelim. Kurallarımızı tanımlamıştık. Bunlarla
birlikte kodumuzu analiz edelim ve çıktımızı görelim.
...
yanlarında ingiliz anahtarı vardır. BU "fixable" olduğunu yani otomatik
düzeltilebildiği anlamına gelir. Kullandığımız iki kural da aslında
bu şekildedir. 

Terminalimizde ESLint çalıştırırken `--fix` bayrağı eklersek
düzeltmeleri yapar ve dosyanızın üzerine yazar. Öncesinde kodumuza son kez bakalım.
...
Halihazırda bahsetmiş olduğumu *ESLint paketi*, terminalden çalıştırdığımız
...
<CustomImage src="/images/post/kurallari-yikmayin-eslint/004.png" width={1460} height={ 364} />

İndirdikten sonra `src/sum.js` dosyamızı en baştaki hale getirelim ve eklentinin
yaptığı sihre odaklanalım.

<CustomImage src="/images/post/kurallari-yikmayin-eslint/008.gif" width={600} height={ 377} />
...
<CustomImage src="/images/post/kurallari-yikmayin-eslint/009.gif" width={800} height={502} />
