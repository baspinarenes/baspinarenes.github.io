...
tag: [variable, function, methods]
---

<div class="warning">
    <p>
    Bu içerik, <a class="warning-link" href="https://twitter.com/leandrotk_"><strong>Leandro TK</strong></a> tarafından yazılmış ve çeşitli platformlarda on binlerce kişiye ulaşmış bir yazıdan çevrilmiştir ve tarafımca bazı düzenlemeler yapılmıştır. Yazının orjinal haline <a class="warning-link" href="https://leandrotk.github.io/2017/09/learning-python-from-zero-to-hero/index.html"><strong>buradan</strong></a> ulaşabilirsiniz.
    </p>
</div>
...
> Tek yazı ile dil öğrenilemeyeceğini, programlama dillerinin de konuştuğumuz dillerde olduğu gibi deneme yanılma süreci gerektirdiğini unutmayın.

Başlamadan önce, yaratıcısı tarafından Python için yapılan tanımı duyalım:

> Programcıların yapacakları şeyleri, birkaç satır kod ile ifade edebilmelerine olanak tanıyan ve bunu yaparken de kodun okunabilirliğinden ödün vermeyen üst düzey programlama dili." - Guido van Rossum
...
> En yakınızdaki marketin ismini düşünün. Kapısı, içerisi hatta bakkalcının duruş şekli bile kafanızda canlandı değil mi? Market binasını değer, marketin ismine değişken diyebiliriz. İsmini duyduğunuzda kafanızda o marketin binası ve konumu canlanacaktır. Değişken mantığı da buna benzerdir.
...
> Hangi döngüyü ne zaman kullanacağımıza dair kabaca konuşacak olursak; yineleme sayısı belliyse <strong>for</strong>, değilse <strong>while</strong> kullanacağız diyebiliriz.
...
Liste veri tipi, değerleri liste olarak saklamak için kullanılabilen bir koleksiyondur (**collection**). Barındırdığı elemanların tipleri aynı olmak zorunda değildir. Basit bir kullanımına bakalım:
...
> Birçok dilde olduğu gibi Python'da da saymaya sıfırdan başlanır. Ekstra olarak Python'da negatif indeksler vardır. Baştan olduğu gibi, sondan da -1, -2, .. şeklinde numaralandırılır.

Kuru kuru geçmektense görselleştirelim ve deneyelim:

<img src="{{ site.baseurl }}/assets/img/posts/python/kus-bakisi-python/indexing.png" style="width: 500px;">
...
<img src="{{ site.baseurl }}/assets/img/posts/python/kus-bakisi-python/slicing.png" style="width: 500px;">
...
> Burada neden bir metod çalıştırıyoruz ki diye düşünebilirsiniz. <strong>getter</strong> kullandığımızda değeri çağırırken başka işlemler de gerçekleştirebiliriz, <strong>setter</strong> kullandığımızda ise atamadan önce kontrol gerçekleştirebiliriz.
...
> Python'da erişimi gerçek anlamda kısıtlayamayız. Erişim belirleyicilerini sezgisel olarak kullanırız, nereden erişebileceğimize alt çizgilere göre biz kendimiz karar veririz. Python denetlemez.
...
Terminali yeniden başlattığınızda her şeyin hazır olduğunu görebilirsiniz. Yazımız burada bitti. Yeni yazılarda görüşmek üzere, esen kalın.